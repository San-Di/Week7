            ...
            public void onResponse(Call<RegisterResponse> call, Response<RegisterResponse> response) {
                ...
                if(registerResponse == null) {
                    ...
                    if(registerResponse.getCode() == MyanmarAttractionsConstants.RESPONSE_CODE_FAIL){
                        UserEvent.FailedRegistrationEvent event = new UserEvent.FailedRegistrationEvent(registerResponse.getMessage());  //the reason of why registerResponse.getMessage but not response.message --> that fail currently occur is because of the code of custom created
                        EventBus.getDefault().post(event);
                    }
            ...
            public void onResponse(Call<LoginResponse> call, Response<LoginResponse> response) {
                ...
                if(loginResponse == null) {
                    ...
                    if(loginResponse.getCode() == MyanmarAttractionsConstants.RESPONSE_CODE_FAIL){
                        UserEvent.FailedRegistrationEvent event = new UserEvent.FailedRegistrationEvent(loginResponse.getMessage());  //the reason of why registerResponse.getMessage but not response.message --> that fail currently occur is because of the code of custom created
                        EventBus.getDefault().post(event);
                    }
                    UserEvent.SuccessRegistrationEvent event = new UserEvent.SuccessRegistrationEvent(loginResponse.getLoginUser());
                    EventBus.getDefault().post(event);
                }
            }
